...
- 품목명 또는 별칭(aliases)이 카탈로그 항목과 일치하면 반드시 카탈로그 이름을 채택합니다.
- 카탈로그에 없는 완전히 새로운 품목만 새 이름을 제안합니다.

## 비교 단위(묶음) 처리 — 매우 중요
입력 항목 중 "비교단위(묶음)=..."로 표시된 것은 사용자가 정한 비교 단위입니다.
- **묶음명이 비교의 주(主) 기준입니다.** 예: A의 "기구부"와 B의 "기구부"를 묶음명으로 매칭.
- 묶음에는 "[참조] 하위 N개: ..." 정보가 있습니다. 이건 보조(補) 참조 정보입니다.
  묶음명만으로 애매할 때, 하위 세부·규격을 참조해 같은 묶음인지 판단하세요.
- 업체마다 구성이 다를 수 있습니다(A는 "차폐"로, B는 "기구부"로 묶음).
  레벨이 달라도 의미가 같으면 묶고, 하위 참조로 확인하세요.
- "품목명=..."으로 표시된 것은 항목(잎) 단위입니다. 규격을 보고 항목끼리 비교하세요.
- 묶음끼리, 항목끼리 비교가 기본입니다. 묶음과 단일 항목이 명백히 같은 것을
  가리키면 묶을 수 있으나, 하위 참조로 신중히 판단하세요.
...
미분류 품목 중 기존 클러스터에 속할 수 있는 것을 찾으세요.

## 판단 기준
- 미분류 품목명이 클러스터 대표명 또는 멤버명과 동일하거나 유사한 경우
- 영문↔한글 번역, 띄어쓰기/괄호 수식어 차이 등도 동일 품목으로 판단

## 엄격 기준 (오탐 방지)
- 확신이 없으면 포함하지 마세요 (오탐 > 미탐)
- 한 품목을 여러 클러스터에 할당하지 마세요
