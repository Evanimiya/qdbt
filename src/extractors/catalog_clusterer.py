...
def run_clustering(
    ...
    if len(submission_items) <= CLUSTER_CHUNK_SIZE:
        return _cluster_single(submission_items, api_key, provider_id, model)
    return _cluster_chunked(submission_items, api_key, provider_id, model)
...
def _cluster_chunked(
    ...
    for chunk in chunks:
        ...
        for w in range(0, len(combined), CLUSTER_CHUNK_SIZE):
            ...
            if len(remaining) >= 2:
                new_clusters = _cluster_single(remaining, api_key, provider_id, model)
