...
## [v0.9.11-test] - 2026-07-10 — 무품명·비연속레벨·번호계층 견고화 + 파이프라인 취약점 일괄 수정

> 상세 변경 이력은 루트 `CHANGELOG.md`의 `[0.9.11]` 항목 참조(0.9.x 라인은 루트가 정본).

- **추출/스티칭**: 품명 열 미지정 시트도 정상 추출(최말단 분류를 잎으로), 비연속 분류 레벨(gap)
  placeholder 보존, 번호(seq) 계층을 구분자 무관(점·하이픈·공백 등) 튜플로 해석.
- **파이프라인 취약점 코드리뷰 일괄 수정**(`docs/QDBT_코드리뷰_취약점_20260710.md`): 숫자·통화
  파서 통합(`core/numparse.py` — 통화기호·괄호음수·유럽식), 외화 원화정합 공용화·스티칭 반영,
  is_total_label 견고화(괄호주석·CJK·영문 오탐), 헤더 자동매핑 부분일치 오탐 제거, LLM 응답
  방어파싱·JSON 견고추출(`core/llm_json.py`), nego 시트경계, residual 절단 통지, 집계 죽은코드·
  중복정의 제거·path 구분자 통일, 추출 프롬프트 도메인/통화 중립화.
- 앱 표시 버전 0.9.4 → 0.9.11.
...
## [v0.7.6-test] - 2026-06-12

### Bug 1: [indent=N] 접두사 비교표 노출 수정

- **원인**: LLM이 들여쓰기 표현을 위해 `[indent=4]L2 Remote Support` 형태로 `name_raw`에 내부 태그 삽입
- `db/queries.py` — `_strip_indent_prefix()` 추가 → `insert_items_bulk()` 저장 시 자동 제거
- `db/queries.py` — `vendor_map`에 `name_normalized` 포함
- `compare/bid.html` — 클러스터 행 품목명을 `name_normalized or name_raw` 우선 표시
- `docs/prompt_extract_v1.md` — `name_raw`에 `[indent=N]` 절대 포함 금지 규칙 추가

### Bug 2: USD 단가 원화 미변환 수정

- **원인**: USD 단가(9,100)가 원화로 변환되지 않아 타 업체 원화 단가와 크게 차이
- `extractors/pipeline.py` — 추출 후처리 추가: `unit_price_currency='USD'`이고 `amount(KRW)` 있으면 `amount ÷ (unit_price × qty)`로 환율 역산, `unit_price` 원화 자동 변환
- `db/queries.py` — 비교 함수에 `fx_rates` 딕셔너리 반환 추가
- `compare/bid.html` — 업체 요약 카드에 💱 환율 배지 표시
- `docs/prompt_extract_v1.md` — USD 병기 처리 규칙 명시

### 미분류 항목 선택 → 클러스터 이동 (v0.7.6)

- 비교 페이지 미분류 행에서 체크박스(행 전체) 또는 셀 클릭(특정 업체)으로 선택
- 상단 액션바에서 기존 클러스터 선택 또는 새 클러스터 생성 후 이동
- `compare.py` — `move_items_to_cluster` 라우트 추가
- `catalog_clusterer.py` — `create_cluster_from_items()`, `move_items_to_cluster()` 추가

---

## [v0.8.5-test] — 2026-06-13 (클러스터링 중복 누적 버그 수정)

### 🐛 핵심 버그: 클러스터링 재실행 시 중복 누적

**증상**: 비교 페이지에서 동일 클러스터("Domestic Travel" 등)가 여러 번 표시.
한 입찰에서 클러스터링 3회 실행 → 클러스터 115개 누적, 86개 항목이 여러 클러스터에 중복 소속.

**원인**: `save_clusters`가 매번 새 ID로 클러스터를 *추가만* 하고 이전 결과를 정리하지 않음.
재실행이 "교체"가 아니라 "누적"으로 동작.

### 수정 내용 (catalog.py `_cluster_worker` + catalog_clusterer.py)

1. **확정 클러스터 항목 제외** — 재클러스터링 대상에서 `status='accepted'` 클러스터에
   속한 항목을 제외하여 확정 작업 보존 + 확정/미확정 간 중복 방지.
2. **재실행 = 교체** — 저장 직전 이전 미확정(`pending`/`held`) 클러스터를 DELETE 후 새로 저장.
3. **거부 보존** — `rejected` 결정은 그대로 유지.
4. **Phase 2·3 확정 보존** — 후속 검증 단계도 확정 클러스터는 건드리지 않음.
5. **동시 실행 잠금** — `_active_cluster_lock` + `_active_cluster_bids`로
   같은 입찰의 클러스터링 동시 실행 차단.

### 신규 함수 (catalog_clusterer.py)

| 함수 | 내용 |
|------|------|
| `_cluster_chunked()` | 품목 90개 초과 시 청크 + carry-forward 증분 클러스터링 |
| `run_unmatched_verification()` | Phase 2 — 미분류 항목이 기존 클러스터에 속하는지 LLM 재검증 |
| `run_cluster_validation()` | Phase 3 — 각 클러스터 구성원 적정성 LLM 재검토 |
| `apply_verification_results()` / `apply_validation_results()` | Phase 2·3 결과 DB 반영 |
| `reopen_cluster()` | 확정/거부/보류 클러스터를 검토 대기로 복원 (catalog_item·price_history 정리) |
| `delete_cluster()` | 클러스터 삭제 (확정 시 연관 catalog_item·price_history 정리) |
| `reset_bid_clusters()` | 입찰의 모든 클러스터 초기화 |

### 데이터 정리
- 기존 누적 중복 제거: "한 항목은 표시 클러스터 하나에만 속함" 원칙
  (확정 > 보류 > 검토대기, 동률이면 최신 실행 우선) → 항목 단위 중복 0개.
- "All-Flash Storage"는 확정/미확정 동명 클러스터 공존 → 가격 이력 보호 위해
  자동 병합하지 않고 병합 기능으로 수동 확인하도록 보존.
