...
def _embed_texts(texts: list, api_key: str):
...
def _cluster_similarities(for_id: str, text_by_cluster: dict):
    ...
    vecs = _embed_texts([text_by_cluster[i] for i in ids], _pick_openai_key())
    if vecs:
        import math
...
def exclude_items_from_cluster(bid_id):
    ...
    try:
        ...
        """, item_ids)
        # submission_items의 match_status도 해제
        conn.execute(f"""
            UPDATE submission_items
            SET catalog_item_id = NULL, match_status = NULL
