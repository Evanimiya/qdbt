...
def compare_bid_submissions(bid_id):
    ...
    with get_conn() as c:
        ...
        for cl in bid_clusters:
            ...
            # 품목명(name_normalized) 기준으로 그룹화 → rowspan 렌더링용
            group_map = {}  # name_key → {vendor → {name_raw, unit_price, item_id}}
            for ci in cluster_items:
                name_key = (ci["name_normalized"] or ci["name_raw"] or "").strip()
                if not name_key:
                    name_key = ci["name_raw"] or "미명명"
                if name_key not in group_map:
                    group_map[name_key] = {}
                group_map[name_key][ci["vendor_name"]] = {
                    "item_id":         ci["item_id"],
            ...
            groups = []
            for gname, vcells in group_map.items():
