...
**모든 분야의 입찰서**를 통합 분석하여 입찰 적정성 판단 및 가격 예측을 지원하는 시스템.

> ⚠️ **설계 원칙**: IT 입찰서로 시작했지만, **설비·용역·IT 전 분야**로 확장을 전제로 설계한다.
> 도메인에 종속된 하드코딩(카테고리명, 프롬프트, 스펙 구조 등)을 피하고,
> 프로젝트/입찰 단위로 도메인을 설정할 수 있는 구조를 지향한다.

### 대상 도메인
```
IT      ← 현재 구현 (서버, 네트워크, SW, 유지보수 등)
설비    ← 향후 (기계설비, 전기설비, 소방설비 등)
용역    ← 향후 (컨설팅, 감리, 유지관리 등)
기타    ← 향후 확장
```

### 3-Proposal 구조 (전 도메인 공통)
```
Commercial Proposal  ← 현재 구현 중 (금액/수량)
Technical Proposal   ← Phase 6 예정 (기술사양)
도면                 ← Phase 6 예정 (치수/구조)
```

### 도메인 확장 시 영향받는 영역
```
카테고리        현재 IT 5개 고정 → 도메인별 카테고리 세트 필요
추출 프롬프트   IT 기준 작성 → 도메인별 프롬프트 분리 필요
카탈로그 스펙   IT 중심 → 도메인별 spec_template 구조 달라짐
비교 기준       단가/수량 → 도메인에 따라 비교 기준 다를 수 있음
```
...
- [x] 프로젝트 목록 / 입찰 목록 화면
- [ ] 프로젝트 도메인 필드 (IT/설비/용역/기타) → **Phase 3-A 이전에 추가 권장**
...
### 3-D: 도메인 확장 대응 ❌
- [ ] 프로젝트 도메인 필드 추가 (IT / 설비 / 용역 / 기타)
- [ ] 도메인별 기본 카테고리 세트
  - IT: 자재/인건비/출장비/영업이익/관리비
  - 설비: 자재/설치/시운전/감리/유지보수/철거
  - 용역: 직접인건비/제경비/기술료/기타
- [ ] 도메인별 추출 프롬프트 분리 (`prompt_extract_facility.md` 등)
- [ ] 도메인별 카탈로그 분리 (설비 품목 ↔ IT 품목 혼재 방지)
