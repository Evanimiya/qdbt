...
MODULES = [s1_classification, s2_currency, s3_tabs, s4_numbering,
           ...
           s11_direct_attach, s12_level_grid, s13_bom, s14_recon]
...
def render(rows):
    ...
    o.append("- **band 요약 고유비용 소실 (S7 7-4) — 해소**: band 요약 행 중 '완전 고립'"
             ...
             "차단. (S7 7-4 PASS)\n")
    o.append("- **시트 간 동일항목·분류명만 상이 (S1 1e) — 해소 (항목 연결 3단계 원칙)**: "
             "`1) 완전합치(코드)`=정규화 경로+품명+규격+금액 정확일치만 자동 dedup. "
             "`2) 유사도(LLM)`=완전합치 안 된 유사항목은 스티칭(무LLM)이 병합·미연계 종결하지 "
             "않고 별도 정본 잎으로 보존(총액 Δ=0)해 DB 저장 → 기존 `catalog_clusterer`(코드 "
             "exact prepass→LLM 유사도 클러스터)·`matcher`(LLM 카탈로그, `suggested`)가 연결 "
             "**제안**. `3) 사람 점검`=담당자 accept/reject 확정. 코드-종결 `cross_sheet_similar` "
             "플래그는 2단계(LLM)를 건너뛴 오종결이라 **제거**. 원칙을 `stitch.py` 배너 주석·"
             "CHANGELOG에 명시. (S1 1e PASS)\n")
