...
## [0.9.14] - 2026-07-14 — 부품 BOM · 재구성층(신규분류·재레벨·레벨엑셀)

v0.9.13 이후 VDI 패치. 견고성 매트릭스 64/64 PASS, 실샘플 15개/27시트 스티칭 총액 **Δ=0**.

- **부품(BOM) 이중 경제구조**: 품목=집계 잎(is_header=0), 부품=품목 하위(레벨6, `bom_part`→is_header=1)
  = **총액 제외**(단가 내역). 롤업 단가=Σ파트금액·금액=수량×단가·**총액=Σ품목금액**(이중계상 방지).
  `submission_items.part_qty/part_price/part_amount`(자동 마이그레이션), `core.totals.check_bom`
  검증(Σ파트≈단가·수량×단가≈금액·per-unit/total 애매성 감지). 추출 자동감지 + BOM 미매핑 시 회귀 없음.
- **업체 비교 '펼쳐보기' 부품 BOM**: 묶음 펼침 모달에서 품목 **-/+ 토글** → 부품 서브행(부품수량·부품단가·
  파트금액) **업체별 비교** + 부품별 최저가. 총액 불참(표시전용).
- **재구성층 오버레이(`map_config.recon`)**: 정본 `submission_items` 불변. `build_items_tree(view=
  'recon'|'original')`가 이동(moves)·신규분류(nodes)·재레벨(level_overrides) 반영. 총액 Δ=0.
  - **#2 신규 분류**: 임의 레벨 '＋ 신규 분류' 생성(`/recon/node`) + 항목/묶음 **비파괴 오버레이 이동**
    (`/recon/move`) + 원본/재구성 뷰 전환(`?view=`) + 초기화(`/recon/reset`). 비교/클러스터링 item_id 무영향.
  - **#4 드래그 재레벨링**: 가지를 레벨 열 헤더에 드롭 → 레벨만 조정(부모 유지·양방향·서브트리 동반·
    역전 방지·부품6 클램프, `/recon/relevel`).
  - **#3 레벨별 엑셀**: `GET /export/level-tree.xlsx` — 열=대/중/소/세/품명/부품 + 규격·수량·단가·금액,
    상위 셀 병합·빈 레벨 빈 칸·부품 합계 제외·합계=Σ품목금액.
- **안정성(chat a)**: 추출 500 → `create_app`에서 idempotent 자동 마이그레이션(`e37759f`), 클러스터링/추출
  GPT `max_tokens` 모델상한 초과 400 자동복구(`e38c4b1`), 비교 그룹총액 중복귀속 제거(`52f6834`),
  묶음 펼치기 팝업을 업체별 캔버스로(`d2724ba`).

## [0.9.13] - 2026-07-13 — 연계 캔버스 UX · 제출서 기본 통화

v0.9.12 이후 VDI 패치. (부품 BOM·#2 재구성층·#3 레벨엑셀·#4 재레벨링은 계획 확인 대기.)

- **선택 노드 스코프 정렬**: 연계 캔버스 정렬을 전역이 아니라 **선택한 노드의 직계 자식에만** 적용
  (레벨별 독립). `sortSib(list, parentPath)` + `S.sortScope`.
- **의미 레벨 열 그리드**: 캔버스를 트리 depth가 아니라 **의미 분류레벨(대/중/소/세/품명)** 고정 열에
  배치. '직접 붙임' 항목은 빈 중간 레벨 열이 보이고 부모→자식 연결선이 그 빈 열을 가로지른다.
  항목에 `submission_items.cat_levels`(JSON, 자동 마이그레이션) 부여 → `build_items_tree` 노드 `level`.
  없으면 depth 폴백(seq/band·구데이터 회귀 없음). 표시 전용·총액 Δ=0.
- **제출서 기본 통화**: `submissions.source_currency`(기본 KRW, 자동 마이그레이션) + detail 통화패널
  드롭다운. 라인에 통화 표기가 없으면 기본 통화로 원통화 지정·원화 재계산(라인 명시 통화 우선,
  되돌리기 가능). 감지 외화 없어도 지정 가능.

## [0.9.12] - 2026-07-12 — 스티칭 3단계 항목연결 · T8 잎펼치기 · 엉망 견적서 견고성 · 중간레벨 직접붙임

v0.9.11 이후 이번 VDI 패치까지의 변경 소급 정리.

- **스티칭 자체 3단계 항목 연결**(코드 완전합치 → LLM 유사 제안 → 사람 확정): 시트 간 유사 후보
  산출(`stitch._detect_link_candidates`) → 온디맨드 라우트 `POST /<sid>/links/suggest`
  (`extractors.stitch_link`, 폐쇄망=휴리스틱 폴백) → residual 패널 '유사 연결 제안' 카드에서
  accept(링크만 Δ=0)/reject/명시적 제외. 자동 병합·종결 금지. 설계·경로검증:
  `docs/QDBT_스티칭_LLM유사연결_계획_20260712.md`.
- **T8 클러스터 잎 펼치기**: 팝업 접이식 트리 + 검색(표시 전용·정본 불변).
- **엉망 견적서 견고성 수정**: 합계행 미탐→이중계상(셀 단독 판정), seq 모드 leaf/breakdown 시트
  은닉 소실→방출+전체요약 roll-up, band 요약 고유비용 보존, 로케일 모호 파싱(단일점 천단위·범위표기),
  정수-seq 이름목록 시트(공종목록) 대분류 소실 복원, 품명 잎이 '소분류'로 오라벨되던 연계 캔버스
  열 헤더 leaf-aware('품명') 수정. 검증: `tests/rob/run_matrix.py` 53/53 PASS,
  실샘플 15개/27시트 스티칭 총액 **Δ=0**.
- **중간 레벨(중/소/세) 부재 표준 = '직접 붙임'**(아래 Changed) — `_prev_val` 상위값 복제 제거,
  규칙 ①~⑤, 잎을 대분류에 직접 잇는 재지정(캔버스 드래그·residual 드롭다운 조상 프리픽스).

> 검증: `tests/rob/run_matrix.py`(S1~S11, 함수 직접 호출·서버 없음) 53/53 PASS,
> 실샘플 15개/27시트 스티칭 총액 **Δ=0**(C2 band 1.65B·C3 seq 1.611B 등 회귀 없음).
> 상세 매트릭스: `docs/QDBT_견고성테스트_20260711.md`.

### Changed — 중간 레벨(중/소/세) 부재 표준 = '직접 붙임' (사용자 확정, 목업 승인)
대>중>소>세>품목에서 중간 레벨이 빠지고 품목이 대분류에 바로 붙는 경우의 처리를 통일했다.
설계: `docs/QDBT_직접붙임_구현_20260712.md`.
- **`extract_by_mapping._prev_val` 상위값 복제 제거**: 대분류 이름을 중/소에 베껴
  `재료비 > 재료비 > 재료비 > 납블록` 가짜 노드를 만들던 레거시 삭제. 빈 중간레벨은 **직접 붙임**.
- **판단 규칙(결정론) ①~⑤**: ① 미매핑 레벨 → 직접(placeholder 없음), ② fill-down 정당 상속 유지,
  ③ 번호는 있는데 이름만 빈 seq 항목(1.2.1) → ⟨이름 누락⟩ 미연계 보존(은닉 소실 방지),
  ④ 형제 비교(같은 부모 형제엔 그 레벨 있는데 이 행만 빔) → placeholder(미연계), 형제 전부 비면 직접,
  ⑤ 그 외 직접. placeholder(⟨미연계·N분류⟩)는 ③④의 '누락 의심'에만.
- **경로 간 통일**: 단일시트 `extract_by_mapping`·다중시트 `stitch._stitch_passthrough` 둘 다 직접
  붙임 기준(비연속 gap placeholder 제거). 트리·캔버스는 **대 > 품목**으로 보존, 잎 열은 '품명' 라벨.
- **UI**: 캔버스에서 잎을 대분류 포함 임의 상위 가지에 드롭 → `path=target` 직접 붙임(Δ=0).
  residual 재지정 드롭다운에 **조상 프리픽스(대분류 등) 직접 선택지** 추가.
- 정본·총액 불변: `_prev_val` 제거로 경로 문자열·depth는 바뀌나 **총액·건수 Δ=0**(실샘플 15개 확인).
